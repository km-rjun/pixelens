//! `fuzzel` launcher backend (a Wayland-native `dmenu` replacement).
//!
//! Feeds the action list to `fuzzel --dmenu` over stdin and reads the chosen
...
/// Map a selected line (verbatim stdout from fuzzel) to a [`MenuChoice`].
///
...
/// True if `fuzzel` is resolvable on `PATH`.
...
impl MenuBackend for FuzzelMenu {
    fn show_menu(&self, _ocr_text: &str) -> Result<MenuChoice, MenuError> {
        let mut child = Command::new("fuzzel")
            ...
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| MenuError::Backend(format!("failed to run fuzzel: {e}")))?;

        child
            .stdin
            .as_mut()
            .ok_or_else(|| MenuError::Backend("fuzzel stdin unavailable".into()))?
            .write_all(ENTRIES.as_bytes())
            .map_err(|e| MenuError::Backend(format!("failed to write to fuzzel: {e}")))?;

        let output = child
            .wait_with_output()
            .map_err(|e| MenuError::Backend(format!("fuzzel failed: {e}")))?;
...
mod tests {
    ...
    #[test]
    fn name_is_fuzzel() {
        assert_eq!(FuzzelMenu.name(), "fuzzel");
    }
    ...
    fn show_menu_reads_shimmed_fuzzel() {
        ...
        let result = FuzzelMenu.show_menu("ocr text").unwrap();
